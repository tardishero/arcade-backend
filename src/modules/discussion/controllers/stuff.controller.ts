...
export class StuffController {
  ...
  async stuffs(): Promise<StuffListResDto> {
    ...
    await Promise.all(
      list.data.map(async (stuff: StuffResDto, index: number) => {
        list.data[index].discussions =
          await this.discussionService.fillWithLikes(stuff.id);
      })
    );
    return list;
  }
  ...
  @Get('/search/:keyword')
  async stuffsBySearch(
    @Param('keyword') keyword: string
  ): Promise<StuffListResDto> {
    const list: StuffListResDto = await this.stuffService.stuffs();
    await Promise.all(
      list.data.map(async (stuff: StuffResDto, index: number) => {
        const discussions = await this.discussionService.discussionByKeyword(
          stuff.id,
          0,
          3,
          keyword
